...
import {getGasCalculation, getGasPrices} from '../../util/gasPriceOracle';
...
class ModalSend extends Component {
  ...
  componentWillMount() {
    try {
      deployed(this.props.web3, 'token').then(async (contracts) => {
        const tokenInstance = contracts.tokenInstance;
        ...
        const gasAmount = await tokenInstance.transfer.estimateGas(address, denormalizeBalance(sum));
